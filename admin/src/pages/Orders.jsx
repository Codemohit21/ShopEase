...
function Orders() {
      ...
      <div className='flex w-full'>
        ...
        <div className='w-[82%] lg:ml-[320px] md:ml-[230px] mt-[70px] flex flex-col gap-6 py-8 px-6 md:px-12'>
          ...
          {orders.length > 0 ? orders.map((order, index) => (
            <div key={index} className='w-full bg-white rounded-xl flex flex-col lg:flex-row items-start lg:items-center justify-between p-4 md:p-6 gap-4 shadow-md hover:shadow-xl transition-transform transform hover:scale-105'>
              
              <SiEbox className='w-[60px] h-[60px] text-white p-2 rounded-lg bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500' />

              <div className='flex-1 flex flex-col gap-2 text-gray-700 text-sm sm:text-base'>
  ...
  className='px-3 py-2 rounded-lg border-none text-white font-semibold shadow-md cursor-pointer transition-transform hover:scale-105'
  style={{
    background: (() => {
      switch (order.status) {
        case "Order Placed":
